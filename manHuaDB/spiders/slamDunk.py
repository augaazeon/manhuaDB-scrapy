...
class SlamdunkSpider(Spider):
    name = 'slamDunk'
    allowed_domains = ['manhuadb.com']
    start_urls = ['https://www.manhuadb.com/manhua/121/']
    ...
    def parse(self, response):
        ...
        chapter = response.xpath(
            '// script[ @ type = "application/ld+json"][2]/text()').extract_first()  # 从<script>提取章节信息和url
    ...
    def parse_item(self, response):
        ...
        for page_info in response.xpath('//option'):
            ...
            yield Request(urljoin('https://www.manhuadb.com', page_url),
                          meta={"chapter_title": chapter_title, "page_num": page_num}, callback=self.parse_image)
    ...
    def parse_image(self, response):
        ...
        item['image_url'] = urljoin('https://www.manhuadb.com',
                                    response.xpath('//img[@class="img-fluid"]/@src').extract_first())
        yield item